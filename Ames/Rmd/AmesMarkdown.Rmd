...
```{r}
colSums(is.na(df2)) #check NA's by column
```
...
df$TotalSqFt <- df3$GrLivArea+df3$TotalBsmtSF+df3$GarageArea
df$TotalPorchSqFt <- df3$OpenPorchSF+df3$EnclosedPorch+df3$ScreenPorch
df$TotalBaths <- df3$BsmtFullBath+df3$BsmtHalfBath+df3$FullBath+df3$HalfBath
df$HouseAge <- as.numeric(df3$YrSold) - as.numeric(df3$YearBuilt)
```
