...
This portion of the analysis, we decided to aggregate some of the variables in order to better represent some of the housing attributes we thought should be examined.  We chose
    ...
    * HouseAge to normalize Year variables
...
Now we'll generate an inital scatterplot matrix to view the untransformed numerical data and see if we find any linear correlation. We picked these variables in order to look at our feature creation and seeing how square footage, in all its forms, plays into the Sale Price.
...
pairs(~SalePrice + TotalSqFt + TotalPorchSqFt + HouseAge + 
        LotFrontage + LotArea + MasVnrArea + PoolArea, data=df3, main="Figure 1: Untransformed Scatterplot")
...
pairs.panels(df3[df3$split == "train",c("SalePrice", "TotalSqFt", "TotalPorchSqFt","HouseAge","LotFrontage", 
             ...
             main="Figure 1A: Untransformed Scatterplot",
             method = "pearson",
             density = TRUE)
...
While there does appear to be some correlation between the area and age-related variables, the data doesn't seem to be normally distributed nor does there seem to be a significant correlation between explanatory variables.  As such, we're going to try a log transformation on SalePrice to see if normality can be improved.

We log-transformed all numeric variables shown in the scatterplot above in advance of our analysis to have them readily available should we need them. To reduce explanatory variables and avoid multi-collinearity issues, we removed the variables used to create the additional features (see "FeatureCreation" above).
...
First we'll try a log-linear approach and only log the dependent variable SalePrice. 
...
pairs(~log_SalePrice + TotalSqFt + TotalPorchSqFt + HouseAge + 
        LotFrontage + LotArea + MasVnrArea + PoolArea, data=df_train, main="Figure2: Log/Linear Scatterplot")

#Different Scatterplot view
pairs.panels(df_train[,c("log_SalePrice", "TotalSqFt", "TotalPorchSqFt","HouseAge","LotFrontage", 
                         "LotArea", "MasVnrArea", "PoolArea")],
             main="Figure 2A: Log/Linear Scatterplot",
             method = "pearson",
             density = TRUE)

```

While we see an improvement in the Log_Sale Price distribution, some of the other features still do not meet normality assumptions, so we'll log them all now to see what we can find. 
...
pairs(~log_SalePrice + log_TotalSqFt + log_TotalPorchSqFt + HouseAge + 
        log_LotFrontage + log_LotArea + log_MasVnrArea + log_PoolArea, data=df_train, main="Figure 3: Log/Log Scatterplot")
...
pairs.panels(df_train[,c("log_SalePrice", "log_TotalSqFt", "log_TotalPorchSqFt","HouseAge","log_LotFrontage", 
             ...
             main="Figure 3A: Log/Log Scatterplot",
             method = "pearson",
             density = TRUE)

```

Now we see the log-log doesn't necessarily improve normality so we will stick with the log-linear transformation for now.  Next lets load a heat map and look at the correlation matrix.
...
corrplot(df_corr, method="circle", order="hclust", addrect=4, win.asp=.7, title="FIgure 4: Variable Corr Heatmap",tl.srt=60)
...
Several explanatory variables show multi-collinearity. These variables provide a list of numeric predictor variables that are moderately or highly correlated to each other, providing an opportunity to decrease redundancy that may skew MLR results.
...
With looking at various categoricals to pick from, trying to find an even blend of saturation across the group means is our goal.  After experimenting with many.  many. combinations of the 79 categorical variables.  We decided on HeatingQC and Central Air. 
