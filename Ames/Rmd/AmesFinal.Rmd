...
For our initial look, we should check out summary stats and see what the means look like across the groups.  Inital graphing showed that the Central Air = No, HeatingQC = Poor categories had very low counts.  We decided to subset that out as that will cause problems with regression. Also, we're starting with the logged SalePrice as our earlier analysis has shown it improves the normal distribution.
...
The means plot above shows us that by looking at the standard deviations 
```{r EDAScatterplots, fig.height=4}
library(gridExtra)
