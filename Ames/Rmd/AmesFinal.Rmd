...
#Just looking at basic means plot we see that the Central Air = Yes category doesn't have a poor rating in any of its heatingQC
...
ggplot(Clean.train, aes(x = HeatingQC, y = log_SalePrice)) +
        geom_point(shape=1) + 
...
ggplot(Clean.train, aes(x = CentralAir, y = log_SalePrice)) +
        geom_point(shape=1) + 
        geom_smooth(method=lm, se=FALSE) +   
        xlab("Central Air") +
        ylab("Sale Price") +
...
ano.fit <-aov(log_SalePrice~HeatingQC+CentralAir,data=Clean.train)
